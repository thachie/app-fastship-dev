...
class PartnerShipmentController extends Controller
{
    ...
    public function storeCreateShipment(Request $request)
    {
        ...
        */
        ...
        if($response === false){
            ...
            $senderEmail = $post['sender_email'];

            //Fastship::getToken($customerId);

            //$response = FS_Pickup::get($pickupId);
            $shipment_data = array();
            $shipment_data[] = FS_Shipment::get($tracking);
            //$shipment_res = FS_Shipment::get($tracking);
            $data = array(
                    'pickupId' => $pickupId,
                    'tracking' => $tracking,
                    'email' => $eMail,
                    'shipmentData' => $shipment_data,
                    //'shipmentRes' => $shipment_res,
                    'sender' => $post,
                    'senderEmail' => $senderEmail,
            );

            if ($senderEmail == '') {
                Mail::send('email/mail_shipment',$data,function($message) use ($data){
                    $message->to($data['email']);
                    $message->bcc(['thachie@tuff.co.th','oak@tuff.co.th']);
                    $message->from('info@fastship.co', 'FastShip');
                    $message->subject('FastShip - พัสดุหมายเลข '. $data['tracking'] ." ถูกสร้างแล้ว");
                });
            }else{
                Mail::send('email/mail_shipment',$data,function($message) use ($data){
                    $message->to($data['email']);
    ...
    public function apiShipmentDetail($id)
    {
        ...
        $qrCode = $this->genQrCodeResize($customerId,trim($id),150);
