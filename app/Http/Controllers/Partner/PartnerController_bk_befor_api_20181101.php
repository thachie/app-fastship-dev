...
class PartnerController extends Controller
{
    ...
    public function prepareCalculateShipmentRatePartner(Request $request)
    {
        ...
        if ($customerId != null){
            //$countries = DB::table('country_fs')->where("IS_ACTIVE",1)->get();
            /*$countries = DB::table('country_fs as c1')
                ->join('country as c', 'c1.CNTRY_CODE', '=', 'c.CNTRY_CODE2ISO')
                ->select(
                    'c1.*',
                    'c.CNTRY_CODE as CODE3'
                    )
                ->where("c.IS_ACTIVE",1)
                ->get();
