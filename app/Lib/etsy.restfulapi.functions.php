...
{ 
	try
	{
		...
		if(!$response){
		   die("Connection Failure or Response data is null!");
...
{ 
	try
	{
		...
		//alert($responseBody = json_decode($response, true));
		...
		alert( "User $user->login_name has a feedback score of {$user->feedback_info->score}");*/
		return $response;
	}catch(Exception $e){
		echo 'Error -- '. $e->getMessage();
	}
